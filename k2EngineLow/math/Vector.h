...
namespace nsK2EngineLow {
	...
	public:
		...
		void MySetRotationDegX(float angle)
		{
			float s;
			float halfAngle = Math::DegToRad(angle) * 0.5f;
			s = sinf(halfAngle);
			w = cosf(halfAngle);
			x = Vector3{ 1.0f, 0.0f, 0.0f }.x * s;
			//y = Vector3{ 1.0f, 0.0f, 0.0f }.x * s;
			//z = axis.z * s;
		}

		void MySetRotationDegZ(float angle)
		{
			float s;
			float halfAngle = Math::DegToRad(angle) * 0.5f;
			s = sinf(halfAngle);
			w = cosf(halfAngle);
			//x = Vector3{ 1.0f, 0.0f, 0.0f }.x * s;
			//y = Vector3{ 0.0f, 0.0f, 1.0f }.z * s;
			z = Vector3{ 0.0f, 0.0f, 1.0f }.z * s;
		}
		...
		void MySetRotationDeg(float angle,float angle2)
		{
			float s;
			float halfAngle = Math::DegToRad(angle) * 0.5f;
			s = sinf(halfAngle);
			w = cosf(halfAngle);
			x = x = Vector3{ 1.0f, 0.0f, 0.0f }.x * s;
			//y = axis.y * s;

			halfAngle = Math::DegToRad(angle2) * 0.5f;
			s = sinf(halfAngle);

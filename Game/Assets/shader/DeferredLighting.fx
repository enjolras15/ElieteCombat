...
float4 PSMainCore(PSInput In, uniform int isSoftShadow) 
{
    ...
    //////////調整用の値//////////
    //フォグを出し始めたい距離
    float fogStartDist = 20000.0f;

	//フォグが最大になる距離
    float fogMaxDist = 40000.0f;
    ...
    //カメラの座標
    float3 cameaXZ = light.eyePos;
    //ワールド上の座標
    float3 worldXZ = worldPos;

    //カメラとワールド上の座標の距離を計算する。
    float dist = length(cameaXZ - worldXZ);

    //フォグの割合を計算
    float fogRate = saturate((dist - fogStartDist) / (fogMaxDist - fogStartDist));
    //累乗していい感じに
    fogRate = pow(fogRate,3);

    if (fogRate >= 0.95)
    {
       // fogColor = float4(1.0f, 1.0f, 0.0f, 1.0f);
    }

    //フォグの割合からフォグ色とブレンド
    finalColor = finalColor * (1.0f - fogRate) + fogColor * fogRate;

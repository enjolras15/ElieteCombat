...
float4 PSMain(SPSIn psIn) : SV_Target0
{
	...
	//////////調整用の値//////////
	//フォグを出し始めたい高さ
	float fogHeight = -3000.0f;

	//フォグを最大にしたい高さ(＝海面の高さ)
	float skyHeight = -8000.0f;

	//fogHeight > skyHeightにする。
	...
	//描画ピクセルの高さ
	float height = psIn.worldPos.y;

	//フォグの割合を計算
	float fogRate = saturate((height - fogHeight) / (skyHeight - fogHeight));

	if (fogRate >= 0.95)
	{
		//fogColor = float4(1.0f, 0.0f, 1.0f, 1.0f);
	}

	//フォグの割合からフォグ色とブレンド
	float4 finalColor = albedoColor * (1.0f - fogRate) + fogColor * fogRate;

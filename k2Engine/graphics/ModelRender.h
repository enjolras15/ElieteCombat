...
namespace nsK2Engine {
	...
	public:
		...
		/// <summary>
		/// スクロールする海を表示したい時の初期化関数 by錦織
		/// </summary>
		/// <param name="initData">モデルデータ。</param>
		void InitSeaModel(const char* filePath,
			std::array<Texture, 2>& normalMaps,
			void* scrollValue,
			int scrollValueSize,
			EnModelUpAxis enModelUpAxis = enModelUpAxisZ,
			bool isShadowReciever = true,
	...
	private:
		...
		/// <summary>
		/// GBuffer描画用の海のモデルを初期化。　by錦織
		/// </summary>
		/// <param name="renderingEngine">レンダリングエンジン</param>
		/// <param name="tkmFilePath">tkmファイルパス</param>
		void InitSeaModelOnRenderGBuffer(
			RenderingEngine& renderingEngine,

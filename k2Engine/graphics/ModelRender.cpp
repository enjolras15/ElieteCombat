...
namespace nsK2Engine {
	...
	void ModelRender::InitSeaModel(const char* filePath,
		std::array<Texture,2>& normalTextures,
		void* scrollValue,
		int scrollValueSize,
		EnModelUpAxis enModelUpAxis,
		bool isShadowReciever,
		bool isFrontCullingOnDrawShadowMap)
	{
		...
		InitSkeleton(filePath);
		// アニメーションを初期化。
		InitAnimation(nullptr, 0, enModelUpAxis);
		// アニメーション済み頂点バッファの計算処理を初期化。
		InitComputeAnimatoinVertexBuffer(filePath, enModelUpAxis);
		// GBuffer描画用のモデルを初期化。
		InitSeaModelOnRenderGBuffer(*g_renderingEngine, normalTextures,scrollValue,scrollValueSize, filePath, enModelUpAxis, isShadowReciever);
		// ZPrepass描画用のモデルを初期化。
		InitModelOnZprepass(*g_renderingEngine, filePath, enModelUpAxis);
		// シャドウマップ描画用のモデルを初期化。
		InitModelOnShadowMap(*g_renderingEngine, filePath, enModelUpAxis, isFrontCullingOnDrawShadowMap);
	...
	void ModelRender::InitSeaModelOnRenderGBuffer(
		RenderingEngine& renderingEngine,
		std::array<Texture,2>& normalTextures,
		void* scrollValue,
		int scrollValueSize,
	...
	{
		ModelInitData modelInitData;
		modelInitData.m_fxFilePath = "Assets/shader/preProcess/RenderToGBufferFor3DSeaModel.fx";
		...
		modelInitData.m_expandConstantBuffer = scrollValue;
		modelInitData.m_expandConstantBufferSize = scrollValueSize;

		//法線テクスチャを指定
		modelInitData.m_expandShaderResoruceView[1] = &normalTextures[0];
		modelInitData.m_expandShaderResoruceView[2] = &normalTextures[1];
